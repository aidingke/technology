<template lang="pug">
	div.personalData
	  ...
	  div.teacherMessage
	     div.title 基本信息
	     ul(v-for="items in arrTeacher" )
	       li {{items.tittle}}
	       li(v-if="items.type!='img'") {{items.content}}
	       li(v-if="items.type=='img'")
	         img(:src="items.content")                     
	  el-col.btnParent(:span="24") 
	    div.btn(:span="24" @click="edit") 编辑   
</template>
<script>
...
export default {
	...
	methods: {
		    ...
		    edit:function(){
		    	this.$router.push({name:'TeacherDeditor'});
		    }
	},
	created:function(){
